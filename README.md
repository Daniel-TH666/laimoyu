...
- 回归检查：`check-i18n.mjs` 的 E 段会断言页面上**不存在指向「本仓库 / Issue 提交」的链接**
  （`github.com/Daniel-TH666`、`github.com/*/*/issues|pull|blob|tree`、`issues/new`、`?template=`），
  以及 `#submit-form` / `#submit-section` 节点不复活。
- ⚠️ **判据不要写成「页面上不许出现 `github.com`」** —— 站点集里**合法收录了 GitHub 官网本身**
  （工具分类的普通条目），西语页因此有 2 个指向 `https://github.com/` 的 `<a>`
  （卡片外链 + 站内指南）。裸 `github.com` 判据会在西语页误报 FAIL，逼你去删一个合法收录的站点。
  E 段早期碰巧只跑日语页（ja 没有该条目）才一直是绿的，改用西语页就会红。
  ⇒ 只匹配「属于我的」标识。同理适用于 `check-leak.py`（它从一开始就是按具体标识写的）。
...
分享入口的 DOM **构建期就写在 6 个语言页里**（爬虫和禁用 JS 的浏览器都能看到结构），
但 HTML 上带着 `hidden` 属性，由 `main.js` 启动时显式摘掉：

- 有 JS → 摘掉 `hidden`，按钮出现
- **没有 JS → 按钮一直隐藏**。这样不会在页面右缘留一个「点了完全没反应」的死按钮

> ⚠️ 所以改这块时记住：**不要改成用 JS 动态 `innerHTML` 插入**。
> 那样静态产物里就没有骨架了，`verify-dist.py` 第 14 段会立刻红。

### 复制的三级降级链

`navigator.clipboard` 并不是到处都能用（非安全上下文 http、权限被拒、老浏览器都会失败）。
实现里按顺序退，**任何一级失败都会明确告诉用户，不做静默失败**：

1. `navigator.clipboard.writeText()` —— 首选（要求安全上下文 + 用户手势 + 文档聚焦）
2. 失败 → 临时 `textarea` + `document.execCommand('copy')` —— 覆盖旧浏览器与 http 页面
3. 仍失败 → 状态条提示**「请手动复制」**，并把那段文案用虚线框显示出来让用户自己选
...
- **字号自适应**：`fitBlock()` 从 92px 起逐步降到 46px（步长 2），直到标题能在限定行数内排下。
  六种语言同一句话的长度差很多（`100 sites to kill time at work` vs `仕事中にサボるための90サイト`），
  写死字号必然在某一种语言上溢出。
- **断行规则**：`tokenize()` 把文本切成「可断行单元」—— CJK / 假名 / 谚文**逐字可断**，拉丁**按词断**；
  遇到单个超长的单元再逐字硬断。直接按空格切会让整段中文变成一行。
- **字体栈**：emoji 字体放在**最后**。前面的字体没有对应码位时会自动往后退，
  于是拉丁/中日韩走系统字体，`🎣` 落到彩色 emoji 字体（放最前面会把所有文字都变成彩色 emoji）。
- **版面不许钉死**：站点面板高度按内容算（`104 + 行数 × 74`），再被「副标题下沿 → 品牌条」
  之间的可用高度夹住，最后在可用区间里垂直居中。曾经把面板钉在 `y=690`，
  结果标题只有一行的语言（中文）在副标题和面板之间堆了一坨约 260px 的空洞。

### 改文案 / 换图片

- 文案全部在语言包的 **`share` 节点**（6 × 24 个 key），不在模板里硬编码。
- 其中两个是**占位符，删了就会变成废话**，`audit-data.py` 会拦：
  - `posterTitle` 必须含 `{count}` —— 少了海报上就没有「多少个站」这个最直观的数字
  - `text` 必须同时含 `{count}` 和 `{url}` —— 少了 `{url}` 复制出去的一整句话里**直接没有链接**
- **标题 / 副标题有长度护栏**（`audit-data.py`，按全宽字符当量算）：
  标题 ≤60、副标题 ≤70。因为 `fitBlock` 在最小字号下仍排不下时会 `.slice(0, maxLines)`
  **静默截断** —— 海报上会出现一句没有省略号的残句，页面上看不出任何异常。
  写文案时别写太长，具体余量跑一下审计就知道。
- 下载文件名来自 `imageName`（英语 `moyu-paradise-en`、中文 `摸鱼乐园`）。
- 链接取的是页面 `<link rel="canonical">`，不是 `location` —— 构建期写死的正式域名比运行时拼更可靠。

### 怎么核对海报「字有没有画全」

**别靠肉眼看缩略图** —— 海报缩到 0.75 倍显示时，汉字笔画会糊在一起。
本项目就踩过一次：中文副标题「一个个**亲手**筛过」被看成「一个**个工**筛过」，
以为 canvas 丢了字，其实是看错了（用下面的工具一量：15 个墨迹分段，与 15 个字符**完全对上**）。

用这个工具，它把指定的一行裁出来放大，并给出**列墨迹分段**（每个字块的位置与宽度）：

```bash
node _diag/zoom-poster.mjs <lang> <y> <h> <zoom>
node _diag/zoom-poster.mjs zh 320 100 3      # 中文海报 y=320 起 100px 高，放大 3 倍
```

输出里会先打印**整图的文字块列表**（`y 起始..结束 / 高度 / 峰值`），用它定位目标行，
再打印该行的分段。**分段数应与该行文案的字符数一致**；对不上就是真丢字了。

> 顺带一个排版自检：底部品牌条的块「y 1196..1363 高 168 峰值 928」正好等于
> `bandH=168` 和 `innerW=928` —— 说明布局算到了底边、且铺满了内容宽度。
...
> - **「配置型」断言要回读实际 key**：种 `localStorage` 时脚本一度写成 `moyu_admin_cfg`，
>   产品里实际是 `moyu_admin_config` → 6 个语言全读到空、统一报 `-1`，
>   看着像产品坏了，其实是脚本自己的键名错。**键名别凭记忆写，从源码里 grep 出来。**
> - ⚠️ **负面断言（「不许出现 X」）比正面断言更容易写错**：写下 `X` 时想的是"某类东西"，
>   而机器只做字面匹配。本项目实测过两次：
>   ① `check-leak.py` 若写成裸 `github.com` 计数 = 0，会在**合法收录 GitHub 官网**的西语页误报；
>   ② 线上核验的 `check-live.py` 曾因 `timeout=40s` 把 `/ko/` 的**网络慢**报成 `HTTP None`。
>   ⇒ **负面断言写完一定先跑一遍，看它报什么；一旦命中，先读上下文再改产品。**
